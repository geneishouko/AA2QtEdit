...
bool DataReader::loadDefinitions(QIODevice *xmlDefinition)
{
    ...
    xml.readNext();
    if(xml.documentVersion() == QLatin1String("1.0")) {
        if(xml.readNextStartElement() && xml.name() == QLatin1String("dataset"))
...
void DataReader::parseDefinitions(QXmlStreamReader &xml) {
    while(xml.readNextStartElement()) {
        if(xml.name() == QLatin1String("enumerables"))
            parseEnumerables(xml);
        else if(xml.name() == QLatin1String("structs"))
            parseStructs(xml);
        else if (xml.name() == QLatin1String("datablocks"))
            parseDatablocks(xml);
        else
...
void DataReader::parseEnumerables(QXmlStreamReader &xml) {
    QString name, type;
    while (xml.readNextStartElement()) {
        Q_ASSERT(xml.name() == QLatin1String("enumerable"));
        ...
        QString enumValue;
        while(xml.readNextStartElement()) {
            Q_ASSERT(xml.name() == QLatin1String("enumpair"));
...
void DataReader::parseStructs(QXmlStreamReader &xml) {
    QString key;
    while (xml.readNextStartElement()) {
        Q_ASSERT(xml.name() == QLatin1String("struct"));
...
void DataReader::parseDatablocks(QXmlStreamReader &xml) {
    int currentOffset = 0;
    while (xml.readNextStartElement()) {
        Q_ASSERT(xml.name() == QLatin1String("datablock"));
